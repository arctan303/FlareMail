...
app.post('/user/addAccount', async (c) => {
	const account = await accountService.adminAdd(c, await c.req.json());
...
app.put('/user/updateSendLimit', async (c) => {
	await userService.updateSendLimit(c, await c.req.json());
	return c.json(result.ok());
});

app.put('/user/updateAccountLimit', async (c) => {
	await userService.updateAccountLimit(c, await c.req.json());
	return c.json(result.ok());
});

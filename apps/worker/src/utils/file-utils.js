const fileUtils = {
	...
	async createAttachmentKey(buff, filename) {
		const hash = await this.getBuffHash(buff);
		const unique = crypto.randomUUID().replace(/-/g, '');
		return `${hash}-${unique}${this.getExtFileName(filename)}`;
	},
	...
	base64ToFile(base64Data, customFilename) {
		const match = base64Data.match(/^data:(image|jpeg|video)\/([a-zA-Z0-9.+-]+);base64,/i);
		if (!match) {
		...
		const cleanBase64 = base64Data.replace(/^data:(image|jpeg|video)\/[a-zA-Z0-9.+-]+;base64,/i, '');
